...
const checkWorld=s=>{const b=M.worldBounds(s);assert.equal(b.right-b.left,1000);assert.equal(b.top-b.bottom,1000);for(const o of s.objects){const q=M.cornerCoords(s,o);assert.ok(Number.isInteger(q.x)&&Number.isInteger(q.y));M.assertWorldPlacement(s,o);}roundTrip(s);};
...
for(const type of ['base','beacon','marshall','center','terrain']){
 ...
 s=M.setObjectCorner(s,o.id,820,920);assert.deepEqual(M.cornerCoords(s,s.objects[0]),{x:820,y:920});assert.deepEqual(M.worldBounds(s),world);checkWorld(s);
 s=M.setObjectCorner(s,o.id,0,0);assert.deepEqual(M.cornerCoords(s,s.objects[0]),{x:0,y:0});checkWorld(s);
...
for(const [w,h] of [[1,1],[4,4],[5,5],[5,6],[6,5],[60,60]]){
 ...
 assert.deepEqual(M.cornerCoords(s,s.objects[0]),{x:820,y:920});checkWorld(s);
 s=M.updateObject(s,o.id,{w:6,h:9});assert.deepEqual(M.cornerCoords(s,s.objects[0]),{x:820,y:920});
 ...
 const moved=M.moveObject(s,o.id,s.objects[0].x+1.4,s.objects[0].y-2.2);assert.deepEqual(M.cornerCoords(moved,moved.objects[0]),{x:821,y:918});checkWorld(moved);
}
...
for(const season of ['4','off']){
 let s=M.setSeason(M.makeLayout('empty'),season),base=M.makeObject(s,'base',20,20);s=M.addObject(s,base);const baseBefore=M.cornerCoords(s,base),world=M.worldBounds(s);
 const anchor=M.makeObject(s,M.anchorType(s),10,0);s=M.addObject(s,anchor);assert.deepEqual(M.cornerCoords(s,base),baseBefore);assert.deepEqual(M.worldBounds(s),world);
 s=M.setObjectCorner(s,anchor.id,820,920);assert.deepEqual(M.cornerCoords(s,s.objects.find(o=>o.id===anchor.id)),{x:820,y:920});assert.deepEqual(M.cornerCoords(s,base),baseBefore);assert.deepEqual(M.worldBounds(s),world);checkWorld(s);
 s=M.removeObject(s,anchor.id);assert.deepEqual(M.cornerCoords(s,base),baseBefore);checkWorld(s);
}
...
for(const gap of [0,1,2]){
 ...
 assert.ok(filled.added>0);checkWorld(filled.state);for(const o of filled.state.objects){const q=M.cornerCoords(filled.state,o);assert.ok(q.x>=984&&q.y>=984&&q.x<=997&&q.y<=997);}
...
let legacy=M.makeLayout('empty');legacy.version=4;legacy.objects.push({...M.makeObject(legacy,'terrain'),x:322.5,y:423,w:5,h:6,terrainGroup:'joined'});legacy.objects.push({...M.makeObject(legacy,'terrain'),x:327.5,y:421.5,w:5,h:3,terrainGroup:'joined'});const legacyBefore=M.clone(legacy),migrated=M.validate(legacy);assert.deepEqual(M.cornerCoords(migrated,migrated.objects[0]),{x:820,y:920});assert.equal(migrated.objects[0].terrainGroup,'joined');assert.deepEqual(legacy,legacyBefore);checkWorld(migrated);
