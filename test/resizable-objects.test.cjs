...
for(const type of ['stronghold','city']){
 let s=M.makeLayout('empty'),o=M.makeObject(s,type);s=M.addObject(s,o);const original=M.cornerCoords(s,o);
 s=M.updateObject(s,o.id,{w:24,h:21,coreW:8,coreH:9});o=s.objects[0];assert.deepEqual(M.cornerCoords(s,o),original);
 const core=M.solidFootprint(o);assert.equal(core.w,8);assert.equal(core.h,9);assert.ok(Number.isInteger(M.cornerCoords(s,core).x)&&Number.isInteger(M.cornerCoords(s,core).y));assert.equal(M.rect(core).left-M.rect(o).left,8);assert.equal(M.rect(core).bottom-M.rect(o).bottom,6);
...
s=M.addObject(s,M.makeObject(s,'base'));const old=M.cornerCoords(s,missile);s=M.updateObject(s,missile.id,{w:50,h:42});missile=s.objects[0];assert.deepEqual(M.cornerCoords(s,missile),old);s=M.moveObject(s,missile.id,1.5,2.5);s=M.moveObject(s,s.objects[1].id,2,3);assert.doesNotThrow(()=>M.validate(s));
