(function(){
'use strict';
const T=globalThis.HiveThemes??{svg:s=>s,init(){}},M=globalThis.HiveModel,I=globalThis.HiveI18n,$=id=>document.getElementById(id),svg=$('map'),stage=$('stage'),t=(k,p)=>I.t(k,p),esc=s=>String(s).replace(/[&<>"']/g,c=>({'&':'&amp;','<':'&lt;','>':'&gt;','"':'&quot;',"'":'&#39;'}[c]));
...
function render(){if(!plan)return;let s='<defs><pattern id="grid" x="-.5" y="-.5" width="5" height="5" patternUnits="userSpaceOnUse"><path d="M5 0H0V5" fill="none" stroke="#263e50" stroke-width=".03"/></pattern></defs>';const world=M.worldBounds(plan);s+=`<rect x="${world.left}" y="${-world.top}" width="1000" height="1000" fill="url(#grid)"/>`;
 for(const o of plan.objects)if(M.coreSize(o))s+=`<rect data-theme-preserve="true" x="${o.x-o.w/2}" y="${-o.y-o.h/2}" width="${o.w}" height="${o.h}" fill="#765638" fill-opacity=".5" stroke="#ad865f" stroke-width=".06"/>`;
 ...
 const seen=new Set();for(const o of [...plan.objects.filter(o=>o.type!=='missile'),...plan.objects.filter(o=>o.type==='missile')]){const name=M.objectLabel(plan,o),q=M.coords(plan,o),c=M.ALLIANCE_COLORS[M.allianceOf(o)-1];
  ...
  s+=`<g ${o.type==='terrain'||core?'data-theme-preserve="true"':''} data-id="${esc(o.id)}"><rect x="${f.x-w/2}" y="${-f.y-h/2}" width="${w}" height="${h}" fill="${fill}" stroke="${o.type==='terrain'?(o.color??'#a8787d'):c}" stroke-width=".07"/>`;
 ...
 svg.innerHTML=T.svg(s);update();updateMissileControls();}
...
$('lang').value=I.language;$('lang').addEventListener('change',e=>{I.setLanguage(e.target.value);I.apply(document);render();search();if(record)$('updated').textContent=t('Stand: {date}',{date:new Date(record.updatedAt).toLocaleString(I.language)});});new ResizeObserver(update).observe(stage);T.init('theme-select',render);I.apply(document);load();
})();
